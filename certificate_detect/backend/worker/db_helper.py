...
class WorkerDB():
    ...
    def get_users(self):
        self.get('users', columns=None, where=None)['data']

    def id_from_queue(self):
        sql_str = 'DELETE queue OUTPUT deleted.* where id = (select min(id) from queue)'
        data = self.request(sql_str)
        return {'session_id': data['session_id'], 'id': data['id'], 'status': data['status']}