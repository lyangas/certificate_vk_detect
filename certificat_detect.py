...
def analyze_users():
    user_ids = request.json['users_ids']
    ...
    for user_id in user_ids:
        ...
        if len(posts_url_and_text) != 0:
            new_user_with_certs = {'user_id': user_id, 'posts_url_and_text': posts_url_and_text}
            #user_with_certs.append(new_user_with_certs)
    
            with open('saved_data/{}/{}.txt'.format(session_name, user_id), 'w') as f:
                f.write(json.dumps(new_user_with_certs))
                
    clusters = analyze(session_name)
