...
class WorkerDB():
    ...
    def __init__(self, dbname='', password='', user='postgres', host='localhost', port=3306):
        self.dbname = dbname
        self.user = user
    ...
    def tables_list(self):
        ...
        with closing(pymysql.connect(db=self.dbname, user=self.user, password=self.password, host=self.host, port=self.port)) as connection:
    ...
    def get(self, table_name, columns=None, where=None):
        ...
        with closing(
                pymysql.connect(db=self.dbname, user=self.user, password=self.password, host=self.host, port=self.port)) as conn:
